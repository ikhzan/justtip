...
import com.justtip.backend.configs.CacheConfig;
...
public class ItemServiceCachingIntegrationTest {
    ...
    void givenRedisCaching_whenFindItemById_thenItemReturnedFromCache(){
        OtherItem anItem = new OtherItem(AN_ID,"on descroption");
        given(mockItemRepo.findById(AN_ID))
                .willReturn(Optional.of(anItem));

        OtherItem itemCacheMiss = itemService.getItemForId(AN_ID);
        OtherItem itemCacheHit = itemService.getItemForId(AN_ID);

        assertThat(itemCacheMiss).isEqualTo(anItem);
        assertThat(itemCacheHit).isEqualTo(anItem);

        verify(mockItemRepo,times(1)).findById(AN_ID);
